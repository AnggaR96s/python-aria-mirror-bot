...
def _generate_id(prefix='saf-'):
    ...
    return prefix + ''.join(choice(chars) for _ in range(25)) + choice(
        chars[1:])
...
def _enable_services(service, projects, ste):
    ...
    for i in projects:
        for j in ste:
            batch.add(service.services().enable(name='projects/%s/services/%s' %
                                                (i, j)))
    batch.execute()
...
if __name__ == '__main__':
    ...
    parse.add_argument('--credentials',
                       default='credentials.json',
                       help='Specify the credentials file path.')
    parse.add_argument('--list-projects',
                       default=False,
                       action='store_true',
    ...
    parse.add_argument('--max-projects',
                       type=int,
                       default=12,
    ...
    resp = serviceaccountfactory(path=args.path,
                                 token=args.token,
